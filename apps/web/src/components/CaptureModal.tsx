...
// available as a modal from anywhere in the app via the global Capture button.
export function CaptureModal() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setResult(null);
    try {
      const res = await captureNote({ rawContent: rawContent.trim(), sourceUrl: sourceUrl.trim() || undefined });
      setResult(res);
      setRawContent("");
      setSourceUrl("");
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/70 px-4 pt-16 sm:pt-24" onClick={close}>
      ...
      >
        <div className="mb-3 flex items-center justify-between">
          ...
          <button type="button" onClick={close} className="text-neutral-500 hover:text-neutral-200">
            <X className="h-4 w-4" />
          </button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-2">
          <textarea
            ...
            rows={4}
            className="w-full rounded-lg border border-neutral-700 bg-neutral-950 px-3 py-2 text-sm text-neutral-100 placeholder:text-neutral-500 transition-colors focus:border-orange-500/60 focus:outline-none focus:ring-2 focus:ring-orange-500/20"
          />
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-lg border border-neutral-700 bg-neutral-950 py-2 pl-9 pr-3 text-sm text-neutral-100 placeholder:text-neutral-500 transition-colors focus:border-orange-500/60 focus:outline-none focus:ring-2 focus:ring-orange-500/20"
            />
          </div>
          <button
            type="submit"
