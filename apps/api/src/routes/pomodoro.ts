...
export async function pomodoroRoutes(app: FastifyInstance) {
  app.get<{ Querystring: { since?: string; todoId?: string } }>("/", async (req) => {
    let sessions;
    if (req.query.todoId) {
      sessions = await pomodoroRepo.listPomodoroSessionsByTodoId(fromGid(req.query.todoId).id, req.userId);
    } else {
      sessions = await pomodoroRepo.listPomodoroSessions(req.userId, req.query.since);
    }
  ...
  app.post("/", async (req, reply) => {
    ...
    const session = await pomodoroRepo.createPomodoroSession(parsed.data, req.userId);
    return reply.code(201).send({ session: serializePomodoroSession(session) });
  });

  app.delete<{ Params: { id: string } }>("/:id", async (req, reply) => {
    const { id } = fromGid(req.params.id);
    const session = await pomodoroRepo.deletePomodoroSession(id, req.userId);
