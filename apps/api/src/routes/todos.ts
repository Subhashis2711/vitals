...
export async function todosRoutes(app: FastifyInstance) {
  app.get<{ Querystring: { sourceNoteId?: string; projectId?: string } }>("/", async (req) => {
    ...
    if (req.query.sourceNoteId) {
      todos = await todosRepo.listTodosBySourceNoteId(fromGid(req.query.sourceNoteId).id, req.userId);
    } else if (req.query.projectId) {
      todos = await todosRepo.listTodosByProjectId(fromGid(req.query.projectId).id, req.userId);
    } else {
      todos = await todosRepo.listTodos(req.userId);
    }
  ...
  app.get<{ Params: { id: string } }>("/:id", async (req, reply) => {
    ...
    const todo = await todosRepo.getTodoById(id, req.userId);
  ...
  app.post("/", async (req, reply) => {
    ...
    const todo = await todosRepo.createTodo(parsed.data, req.userId);
  ...
  app.delete<{ Params: { id: string } }>("/:id", async (req, reply) => {
    const { id } = fromGid(req.params.id);
    const todo = await todosRepo.deleteTodo(id, req.userId);
    if (!todo) return reply.code(404).send({ error: "Todo not found" });
    return { todo: serializeTodo(todo) };
  });
  ...
  app.post("/reorder", async (req, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: parsed.error.flatten() });
    const result = await todosRepo.swapTodoPositions(parsed.data.firstId, parsed.data.secondId, req.userId);
    if (!result) return reply.code(404).send({ error: "Todo not found" });
