...
export async function healthRoutes(app: FastifyInstance) {
  app.get<{ Querystring: { since?: string } }>("/daily", async (req) => {
    ...
    const logs = await healthRepo.listDailyLogsSince(since, req.userId, req.workspaceId);
    return { logs };
  });
  ...
  app.post("/daily", async (req, reply) => {
    ...
    const log = await healthRepo.upsertDailyLog(parsed.data, req.userId, req.workspaceId);
    return reply.code(201).send({ log });
  });

  app.get<{ Querystring: { since?: string } }>("/activity", async (req) => {
    const since = req.query.since ?? defaultSinceDate(30);
    const logs = await healthRepo.listActivityLogsSince(since, req.userId, req.workspaceId);
    return { logs };
  });
  ...
  app.post("/activity", async (req, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: parsed.error.flatten() });
    const log = await healthRepo.createActivityLog(parsed.data, req.userId, req.workspaceId);
    return reply.code(201).send({ log });
  });

  app.delete<{ Params: { id: string } }>("/activity/:id", async (req, reply) => {
    const log = await healthRepo.deleteActivityLog(req.params.id, req.userId, req.workspaceId);
