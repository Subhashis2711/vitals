...
export async function moneyRoutes(app: FastifyInstance) {
  app.get("/transactions", async (req) => {
    const transactions = await moneyRepo.listTransactions(req.userId);
    return { transactions };
  });
  ...
  app.post("/transactions", async (req, reply) => {
    ...
    const transaction = await moneyRepo.createTransaction(parsed.data, req.userId);
    return reply.code(201).send({ transaction });
  });

  app.delete<{ Params: { id: string } }>("/transactions/:id", async (req, reply) => {
    const transaction = await moneyRepo.deleteTransaction(req.params.id, req.userId);
    if (!transaction) return reply.code(404).send({ error: "Transaction not found" });
    return { transaction };
  });

  app.get("/savings-goals", async (req) => {
    const savingsGoals = await moneyRepo.listSavingsGoals(req.userId);
    return { savingsGoals };
  });
  ...
  app.post("/savings-goals", async (req, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: parsed.error.flatten() });
    const savingsGoal = await moneyRepo.createSavingsGoal(parsed.data, req.userId);
  ...
  app.patch<{ Params: { id: string } }>("/savings-goals/:id", async (req, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: parsed.error.flatten() });
    const savingsGoal = await moneyRepo.updateSavingsGoal(req.params.id, parsed.data, req.userId);
  ...
  app.delete<{ Params: { id: string } }>("/savings-goals/:id", async (req, reply) => {
    const savingsGoal = await moneyRepo.deleteSavingsGoal(req.params.id, req.userId);
    if (!savingsGoal) return reply.code(404).send({ error: "Savings goal not found" });
    return { savingsGoal };
  });
}
