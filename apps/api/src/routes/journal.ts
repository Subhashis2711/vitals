...
export async function journalRoutes(app: FastifyInstance) {
  app.get("/", async (req) => {
    const entries = await journalRepo.listJournalEntries(req.userId, req.workspaceId);
  ...
  app.get<{ Querystring: { date?: string } }>("/by-date", async (req, reply) => {
    ...
    const entry = await journalRepo.getJournalEntryByDate(req.query.date, req.userId, req.workspaceId);
  ...
  app.post("/", async (req, reply) => {
    ...
    const entry = await journalRepo.upsertJournalEntry(parsed.data, req.userId, req.workspaceId);
    return reply.code(201).send({ entry: serializeJournalEntry(entry) });
  });

  app.delete<{ Params: { id: string } }>("/:id", async (req, reply) => {
    const { id } = fromGid(req.params.id);
    const entry = await journalRepo.deleteJournalEntry(id, req.userId, req.workspaceId);
