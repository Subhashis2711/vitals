...
export async function goalsRoutes(app: FastifyInstance) {
  app.get<{ Querystring: { projectId?: string; topicId?: string } }>("/", async (req) => {
    ...
    if (req.query.projectId) {
      goals = await goalsRepo.listGoalsByProjectId(fromGid(req.query.projectId).id, req.userId);
    } else if (req.query.topicId) {
      goals = await goalsRepo.listGoalsByTopicId(fromGid(req.query.topicId).id, req.userId);
    } else {
      goals = await goalsRepo.listGoals(req.userId);
    }
  ...
  app.get<{ Params: { id: string } }>("/:id", async (req, reply) => {
    ...
    const goal = await goalsRepo.getGoalById(id, req.userId);
    if (!goal) return reply.code(404).send({ error: "Goal not found" });
    const todos = await goalsRepo.listGoalTodos(id, req.userId);
  ...
  app.post("/", async (req, reply) => {
    ...
    const goal = await goalsRepo.createGoal(parsed.data, req.userId);
    if (goal.topicId) await learningRepo.touchTopic(goal.topicId, req.userId);
  ...
  app.patch<{ Params: { id: string } }>("/:id", async (req, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: parsed.error.flatten() });
    const { id } = fromGid(req.params.id);
    const goal = await goalsRepo.updateGoal(id, parsed.data, req.userId);
    if (!goal) return reply.code(404).send({ error: "Goal not found" });
    if (goal.topicId) await learningRepo.touchTopic(goal.topicId, req.userId);
  ...
  app.delete<{ Params: { id: string } }>("/:id", async (req, reply) => {
    const { id } = fromGid(req.params.id);
    const goal = await goalsRepo.deleteGoal(id, req.userId);
    if (!goal) return reply.code(404).send({ error: "Goal not found" });
    return { goal: serializeGoal(goal) };
  });
}
