...
async function withProgress(rows: GoalRow[]) {
  const db = getDb();
  const results = [];
  for (const goal of rows) {
    const linked = await db.select().from(todos).where(eq(todos.goalId, goal.id));
...
export async function getGoalById(id: string, userId: string) {
  ...
  if (!row) return null;
  const [withProg] = await withProgress([row]);
  return withProg;
}

export async function listGoalTodos(goalId: string, userId: string) {
  const db = getDb();
  return db.select().from(todos).where(and(eq(todos.goalId, goalId), eq(todos.userId, userId)));
}

export async function listGoalsByProjectId(projectId: string, userId: string) {
  const db = getDb();
  const rows = await db
    .select()
    .from(goals)
...
export async function listGoalsByTopicId(topicId: string, userId: string) {
  const db = getDb();
  const rows = await db
    .select()
    .from(goals)
    .where(and(eq(goals.topicId, topicId), eq(goals.userId, userId)))
...
export async function createGoal(input: CreateGoalInput, userId: string) {
  ...
  if (position === undefined && input.topicId) {
    const existing = await db
      .select()
      .from(goals)
      .where(and(eq(goals.topicId, input.topicId), eq(goals.userId, userId)));
