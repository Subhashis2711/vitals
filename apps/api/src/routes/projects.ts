...
export async function projectsRoutes(app: FastifyInstance) {
  app.get("/", async (req) => {
    const projects = await projectsRepo.listProjects(req.userId);
    return { projects: projects.map(serializeProject) };
  });

  app.get<{ Params: { id: string } }>("/:id", async (req, reply) => {
    const { id } = fromGid(req.params.id);
    const project = await projectsRepo.getProjectById(id, req.userId);
  ...
  app.post("/", async (req, reply) => {
    ...
    const project = await projectsRepo.createProject(parsed.data, req.userId);
  ...
  app.patch<{ Params: { id: string } }>("/:id", async (req, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: parsed.error.flatten() });
    const { id } = fromGid(req.params.id);
    const project = await projectsRepo.updateProject(id, parsed.data, req.userId);
    if (!project) return reply.code(404).send({ error: "Project not found" });
    return { project: serializeProject(project) };
  });
  ...
  app.delete<{ Params: { id: string } }>("/:id", async (req, reply) => {
    const { id } = fromGid(req.params.id);
    const project = await projectsRepo.deleteProject(id, req.userId);
    if (!project) return reply.code(404).send({ error: "Project not found" });
    return { project: serializeProject(project) };
  });
}
