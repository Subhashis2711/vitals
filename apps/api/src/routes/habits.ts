...
export async function habitsRoutes(app: FastifyInstance) {
  app.get("/", async (req) => {
    const habits = await habitsRepo.listHabits(req.userId);
    return { habits };
  });
  ...
  app.post("/", async (req, reply) => {
    ...
    const habit = await habitsRepo.createHabit(parsed.data, req.userId);
    return reply.code(201).send({ habit });
  });

  app.delete<{ Params: { id: string } }>("/:id", async (req, reply) => {
    const habit = await habitsRepo.deleteHabit(req.params.id, req.userId);
  ...
  app.get<{ Querystring: { since?: string } }>("/logs", async (req) => {
    ...
    const logs = await habitsRepo.listHabitLogsSince(since, req.userId);
    return { logs };
  });

  app.post<{ Params: { id: string } }>("/:id/toggle", async (req, reply) => {
    const habit = await habitsRepo.getHabitById(req.params.id, req.userId);
    if (!habit) return reply.code(404).send({ error: "Habit not found" });

    const parsed = toggleHabitLogInputSchema.safeParse(req.body ?? {});
    if (!parsed.success) return reply.code(400).send({ error: parsed.error.flatten() });

    return habitsRepo.toggleHabitLog(req.params.id, req.userId, parsed.data.date);
  });
}
